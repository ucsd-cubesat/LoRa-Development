/* Tyler Eaves
   ...
   This file has been derived from the original lora.c which was used
   for testing SPI with the SX1278 as well as testing SX1278 
   functionality.  This file, loraTX.c, is intended to grow into a 
   state machine that will control data transmissions on board a
   balloon. This program is designed to run on a Raspberry Pi 
   using the bcm2835 library. It can be compiled with something like:

   $ cc loraTX.c -o loraTX -lbcm2835
   ...
   I've successfully received a complete packet using this test transmission 
   code and some test receiver code on another RPI and SX1278 module across the 
   room.  The problem I had to solve was yet another ambiguity in the the SX1278 
   datasheet.  See the description of the register holding payload length, 0x22, 
   on p. 133. Skimming over this one might think this register should be untouched
   unless operating in implicit header mode, however these datasheet notes refer
   only to receiver functionality.  The SX1278 chip defaults to explicit header
   mode, and regularly the payload length only needs to be set on the transmit
   side.  The receiver will detect any length payload as long as it's less than the
   maximum configured value.  This register's disclaimer states that the only time 
   the payload length needs to be specified at the receiver module is when operating
   in implicit header mode.  The same is NOT true on the transmit side.  Regardless 
   of implicit/explicit mode the payload length must ALWAYS be specified on the 
   transmit side.  If it's configured incorrectly you'll simply transmit the wrong
   number of bytes! 

   I've enabled RPI system time checking using the get_time() helper function, and 
   am using it as a payload for battery life tests. The idea is to monitor the 
   number of transmissions at regular intervals to calculate battery duration under
   load.  I'll be directing these timestamp payloads to the "stdout" filestream 
   and redirecting them to the file batlife.txt in the same directory as this file. 
   This behavior will be scripted to run on boot via the "rc.local" file.     
...
//helpful values
#define CLEAR_IRQ_FLAGS   0b11111111  //0xFF
#define FLAG_TX_DONE      0b00001000  //0x08
...
uint8_t write_reg(uint8_t addr, char data); 
...
int main(int argc, char **argv){
  ...
  hardware_init();  //setup spi
  lora_init();      //boot sequence to LoRa standby
  //diagnose();       //read relevant LoRa registers for diagnostics

  //allocates space for date/time info
  char payload[25];

  //set transceiver payload length
  write_reg(REG_PAYLOAD_LEN, (sizeof(payload) - 1));
  ...
  while(1){
    ...
    //get data and define the payload
    *strcpy(payload, get_time());
    ...
    clock_t timer1 = 0;
    for(clock_t start = clock(); timer1/CLOCKS_PER_SEC <= 0.5; timer1 = clock() - start){}

    //confirm Tx
    if(read_reg(REG_IRQ_FLAGS)==FLAG_TX_DONE && read_reg(REG_OP_MODE)==LORA_STANDBY){
      write_reg(REG_IRQ_FLAGS, CLEAR_IRQ_FLAGS);
      printf("Transmitted payload: ");
      print_array(payload, sizeof(payload) - 1);
    }

    //4.5 seconds for a total 5 second transmission delay
    clock_t timer2 = 0;
    for(clock_t start = clock(); timer2/CLOCKS_PER_SEC <= 4.5; timer2 = clock() - start){}
  }
...
//recently updated second arg type from uint8_t to char.
//this allows direct entry in the form 'h' rather than hex or binary
//however, entering hex or binary for configuring hardware still works the same
uint8_t write_reg(uint8_t addr, char data){
  ...
  /*
  if(addr == 0x00){
    printf("Wrote character %c to FIFO.\n", data);
  }else{
...
//establishes spi and configures appropriate bit transfer parameters
void hardware_init(void){
...
//I need to update this function to enable a "retry" functionality
//if it doesn't work, try again. Don't just cancel
void lora_init(void){ 
...
//checks current system time, stores in array and returns
//its address.  payload is one element longer than number of
//information bytes to leave room for null terminator.
//sprintf effectively copies a string into array by
//printing "to string"
char* get_time(void){
  time_t rawtime;
  struct tm * timeinfo;
  ...
  static char payload[25];  //must be static to return local variable addr
  sprintf(payload, "%s", asctime(timeinfo));  
  return payload;
}
